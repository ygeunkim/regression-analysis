...
```{proof}
From Equation \@ref(eq:pcralpha) and Equation \@ref(eq:svdlse),

\begin{equation}
  \hb_{A}^{PCR} = V \hat{\boldsymbol\alpha} = VD^{-1}U^T \mathbf{Y} = \hb_{A}^{OLS}
  (\#eq:pcrlse)
\end{equation}
```
...
Recall that $\overline{Y}$ is an estimate for $\beta_0$, not just a constant. So it would be removed when differentiating w.r.t. $\B_A$. Then we now have
