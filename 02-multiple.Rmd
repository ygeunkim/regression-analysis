...
In experimental design literature, effects model is more frequently used. Decomposing each group mean with overall mean and *treatment effect*, it gives more intuitive decription [@Montgomery:2012aa]. So we look at this effects model first.
...
Look at the model matrix $X = \begin{bmatrix} \mathbf{1} \mid \mathbf{x}_1 \mid \cdots \mid \mathbf{x}_k \end{bmatrix} \in \R^{N \times (k + 1)}$, where $N = \sum n_j$. The qualitative variable with level $k$ has made $k$ dummy variables

$$
E_j = \begin{cases}
  1 & \text{from the treatment} \\
  0 & \text{otherwise}
\end{cases}
$$

This is a bit different with dummy coding \@ref(moddummy1) producing $k - 1$ variables. Note that, however,
...
$$\B \equiv (\mu_1, \ldots, \mu_k)^T \in \R^k$$

One proceeds in a similar way for this $\B$,
