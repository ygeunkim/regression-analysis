...
$$\B \equiv (\mu, \tau_1, \ldots, \tau_k)^T$$

Then we can build the regression model as
...
i.e. linearly independent. This design matrix $X$ is of rank deficient. Hence, the normal equation $(X^TX)^{-1}\hb = X^T \mathbf{Y}$ does *not have a unique solution*. This kind of coding is not appropriate. @Rawlings:2006aa provides four ways *reparameterizing* to remove singularities.
...
$$\B^{\ast} \equiv (\mu_1, \ldots, \mu_k)^T \in \R^k$$

One proceeds in a similar way for this $\B^{\ast}$,
...
\end{bmatrix}} = \underset{\huge X^{\ast}}{\begin{bmatrix}
  ...
  \vdots & \vdots & \vdots & \vdots & \vdots \\
  0 & 0 & 0 & \cdots & 1
\end{bmatrix}} \underset{\huge \B^{\ast}}{\begin{bmatrix}
  \mu_1 \\
  \mu_2 \\
  \vdots \\
  \mu_k
...
where $N = \sum n_j$. Note that $\{ \mathbf{x}_1, \ldots, \mathbf{x}_k \}$ is orthogonal. Normalize each column matrix by
...
$$\hb = R^{-1}Q^T\mathbf{Y}^T = (\overline{Y}_{1.}, \overline{Y}_{2.}, \ldots, \overline{Y}_{k.})^T$$

where $\overline{Y}_{j.} = \sum\limits_{i = 1}^{n_j} Y_{ij}$, $j$-the treatment mean.
...
$$\B^{\ast} \equiv (\mu, \tau_1, \ldots, \tau_{k - 1})^T \in \R^k$$
...
with $X^{\ast} \in \R^{N \times k}$. This is called *effect coding*.
...
\begin{equation*}
  \begin{split}
    X^{\ast} \hb^{\ast} & = \hat\mu \mathbf{1} + \mathbb{X}_A \boldsymbol{\tau} \quad \text{where}\: \boldsymbol{\tau} = (\tau_1, \ldots, \tau_{k - 1})^T \\
...
where $X^{\ast} = \begin{bmatrix} \mathbf{1} \mid \mathbf{x}_1 \mid \cdots \mid \mathbf{x}_{k - 1} \end{bmatrix} \in \R^{N \times k}$. Let
...
\begin{equation*}
  \begin{split}
    R^{-1} & = \left[\begin{array}{cccc|cccc}
