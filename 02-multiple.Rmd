...
where $X^{\ast} = \begin{bmatrix} \mathbf{1} \mid \mathbf{x}_1 \mid \cdots \mid \mathbf{x}_{k - 1} \end{bmatrix} \in \R^{N \times k}$.

Denote that every column but the first one is orthogonal. Applying *G-S process* to this $\mathbf{1}$,

$$\mathbf{v} = \mathbf{1} - (1, 1, \ldots, \underset{n_k}{\underline{0, \ldots, 0}})^T = (0, 0, \ldots, \underset{n_k}{\underline{1, \ldots, 1}})^T$$

Let $X^{\ast} = \begin{bmatrix} \mathbf{v} \mid \mathbb{X}_A \end{bmatrix}$. Then

$$
\begin{equation*}
  \begin{split}
    X^{\ast} \hb^{\ast} & = \hat\mu \mathbf{v} + \mathbb{X}_A \boldsymbol{\tau} \quad \text{where}\: \boldsymbol{\tau} = (\tau_1, \ldots, \tau_{k - 1})^T \\
    & = \mathbf{v} \Pi(\mathbf{Y} \mid R(\mathbf{1})) + \mathbb{X}_A \Pi(\mathbf{Y} \mid R(\mathbb{X}_A)) \\
    & = \mathbf{v}(\mathbf{v}^T \mathbf{v})^{-1} \mathbf{v}^T \mathbf{Y} + \mathbb{X}_A (\mathbb{X}_A^T \mathbb{X}_A)^{-1} \mathbb{X}_A^T \mathbf{Y}
  \end{split}
\end{equation*}
$$
...
$$
\mathbb{X}_A^T \mathbb{X}_A = \left[\begin{array}{cccc|ccc}
  n_1 & 0 & \cdots & 0 & 0 & \cdots & 0 \\
  0 & n_2 & \cdots & 0 & 0 & \cdots & 0 \\
  \vdots & \vdots & \cdots & \vdots & 0 & \cdots & 0 \\
  0 & 0 & \cdots & n_{k - 1} & 0 & \cdots & 0 \\ \hline
  \vdots & \vdots & \cdots & \vdots & 0 & \cdots & 0 \\
...
\begin{equation*}
  \begin{split}
    \boldsymbol{\hat\tau} & = (\mathbb{X}_A^T \mathbb{X}_A)^{-1} \mathbb{X}_A^T \mathbf{Y} \\
    & = diag\Big(\frac{1}{n_1}, \ldots, \frac{1}{n_{k - 1}}, \underset{N - k + 1}{\underline{0, \ldots, 0}} \Big) \mathbb{X}_A^T \mathbf{Y} \\
    & = 
  \end{split}
\end{equation*}
$$
