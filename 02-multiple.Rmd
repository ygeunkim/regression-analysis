...
In fact, this example might not appropriate, so we consider another example. This is also from @Chatterjee:2015aa.

```{r}
employ <- haven::read_sav("data/p134.sav")
(employ <-
  employ %>% 
...
- `TEST`: test scores
- `RACE`: race
- `JPERF`: index of job performance

```{r}
(job_fit <- lm(JPERF ~ TEST * RACE, data = employ))
```
...
employ %>% 
  data_grid(TEST, RACE) %>% 
  add_predictions(job_fit) %>% 
  ggplot() +
  geom_line(aes(x = TEST, y = pred, colour = RACE)) +
  geom_point(
    data = employ, 
    aes(x = TEST, y = JPERF, colour = RACE), 
    alpha = .7, 
    show.legend = FALSE
  ) +
  labs(
