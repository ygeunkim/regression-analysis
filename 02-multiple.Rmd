...
$$\Big\{ \B_A : (\hb_A - \B_A)^T \mathbb{X}_{A, \perp}^T\mathbb{X}_{A, \perp} (\hb_A - \B_A) \le p \hat\sigma^2 F_{1 - \alpha}(p, n - p - 1) \Big\}$$
...
```{r essfig, echo=FALSE, fig.cap="Extra sum of squares"}
knitr::include_graphics("images/multiple-extrass.png")
```

$SST$ does not change given data. By adding some variable, $SSR$ increases or $SSE$ decreases. *We interpret this change as additional contribution of those variables*. If it is large enough, they are significant. Using this idea, we might test any kind of sets of coefficients. Partial $F$-test and sequential $F$-test are typical forms that we can think of and useful.
...
$$\mathbf{Z} \equiv \sigma (C(X^TX)^{-1}C^T)^{-\frac{1}{2}} (C \hat{\B} - \mathbf{d}) \hsim MVN(\mathbf{0}, I)$$
