...
### Orthogonal decomposition {#orthogonaldecomp}
...
alggs <- alg_num
alggsstr <- "
| **INPUT:** $\\{ \\mathbf{x}_0, \\ldots, \\mathbf{x}_p \\}$;
...
| **end**
| "
alggsstr <- paste0(alggsstr, "Algorithm", alggs, ": Gram-schmidt process")
```
...
| **end**
| "
algqrstr <- paste0(algqrstr, "Algorithm", algqr, ": QR decomposition for modified G-S process")
```
...
`r if(!is_latex) algqrstr`

This *orthonormal basis* gives some useful facts with least squares problem [@Leon:2014aa].

### Successive orthogonalization

In fact, G-S process `r if (is_latex) {"\\@ref(alg:alggs)"} else alggs` is equivalent to succesive orthogonalization, i.e. regress(project) $\mathbf{x}_j$ onto the others [@Hastie:2013aa].
...
|   $\\hat{\\beta}_{lk} = \\frac{< \\mathbf{v}_l, \\mathbf{x}_k>}{< \\mathbf{v}_l, \\mathbf{v}_l>}, l = 0, \\ldots, k - 1$;
|   Residual $\\mathbf{v}_k = \\mathbf{x}_k - \\sum\\limits_{l = 0}^{k - 1} \\hat\\beta_{lk} \\mathbf{v}_k$;
| **end**
| "
algorthstr <- paste0(algorthstr, "Algorithm", algorth, ": Successive orthogonalization")
```
...
| $j \\leftarrow 1$
| **Repeat**
|   $FM : Y_i = \\beta_0 + \\cdots + \\beta_j x_{1j} + \\epsilon_i$ vs $RM : Y_i = \\beta_0 + \\ldots + \\beta_{j - 1} x_{1, j - 1} + \\epsilon_i$;
...
| "
algseqstr <- paste0(algseqstr, "Algorithm", algseq, ": Sequential F Test from beginning")
```
...
`r if(!is_latex) algseqstr`


Algorithm `r if (is_latex) {"\\@ref(alg:algseq)"} else algseq` shows the logic how to add a variable sequentially. It continues until rejecting $H_0$. In general form, we can test
...
### Interaction variables {#moddummy2}
...
```{r saldfit2, fig.cap="Interaction between qualitative variables"}
salary_relev %>% 
  data_grid(X, E, M) %>% 
...
### General models with interaction

In the previous section \@ref(moddummy1) and \@ref(moddummy2), there have been some assumptions in the model. In Model \@ref(eq:dummymod), *additive assumption* is given. Effects of each predictors do note depend on the others. It means that in the separate model setting \@ref(modsep) we give conditions
...
Next model \@ref(eq:dummymod2) quite weakens this assumption, i.e. remove the second condition for $\delta$ by adding an interaction term between qualitative predictors. Instead, it still assumes that *every combination of levels always has same slope* $\beta_1$. Now we want more general one without the first assumption. Add interaction between $x_i$ and other dummy variables.
...
In the same process, we have separate model. Since we have added terms with $x_i$, the only change from Equation \@ref(eq:dummysep2) is slope part.
...
```{r saldfit3, fig.cap="Full interaction model"}
salary_relev %>% 
  data_grid(X, E, M) %>% 
...
```{r, include=FALSE}
medicine <-
  tibble(trt = gl(3, k = 30, labels = 1:3)) %>% 
  group_by(trt) %>% 
  mutate(y = rnorm(n = 30, mean = as.numeric(trt))) %>% 
  ungroup()
```
...
In experimental design literature, effects model is more frequently used. Decomposing each group mean with overall mean and *treatment effect*, it gives more intuitive decription [@Montgomery:2012aa]. So we look at this one first.
...
Note that we have $k + 1$ parameters in the model \@ref(eq:effmod). Write the parameter vector as
...
Look at the model matrix $X = \begin{bmatrix} \mathbf{1} \mid \mathbf{x}_1 \mid \cdots \mid \mathbf{x}_k \end{bmatrix} \in \R^{N \times (k + 1)}$, where $N = \sum n_j$. From the second column to the last, $\mathbf{x}_j = (x_{1j}, \ldots, x_{n_j, j})$ has value of
...
i.e. column vectors are linearly dependent. This design matrix $X$ is of rank deficient. Hence, the normal equation $(X^TX)^{-1}\hb = X^T \mathbf{Y}$ does *not have a unique solution*. This kind of coding is not appropriate. There are four ways of *reparameterizations* to remove singularities [@Rawlings:2006aa].
...
With this model \@ref(eq:meanmod), we test if every $\mu_j$ is identical.

$$H_0: \mu_1 = \cdots = \mu_k \quad \text{vs} \quad H_1: \text{not}\: H_0$$

Write a parameter vector by

$$\boldsymbol\theta^{\ast} \equiv (\mu_1, \ldots, \mu_k)^T \in \R^k$$
...
$$R = diag(\sqrt{n_1}, \sqrt{n_2}, \ldots, \sqrt{n_k}) \in \R^{k \times k}$$
...
$$\hb = (\overline{Y}_{1.}, \overline{Y}_{2.}, \ldots, \overline{Y}_{k.})^T$$
...
<!--     sse = sum( (y - mean(y))^2 ) -->
<!--   ) %>%  -->
<!--   summarise_at(vars("ssr", "sse"), list(~sum)) -->
<!-- ``` -->

### Effect coding

Go back to the effects model \@ref(eq:effmod). In fact, this model should be constrained by *identifiability condition* such as
...
$$\boldsymbol\theta^{\ast} \equiv (\mu, \tau_1, \ldots, \tau_{k - 1})^T \in \R^k$$
...
with $X^{\ast} \in \R^{N \times k}$. This is called *effect coding*. From the second column of design matrix, the variable is coded by
...
```{r}
C(medicine$trt, contr = contr.sum)
```
...
where $X^{\ast} = \begin{bmatrix} \mathbf{1} \mid \mathbf{x}_1 \mid \cdots \mid \mathbf{x}_{k - 1} \end{bmatrix} \in \R^{N \times k}$. This makes $k - 1$ indicator variables such that
...
\begin{equation*}
  \begin{split}
   & X^{\ast T} X^{\ast} \B = X^{\ast T} \mathbf{Y} \\
...
<!-- $$\mathbf{u}_j = \frac{\mathbf{x}_j}{\lVert \mathbf{x}_j \rVert} = \frac{1}{\sqrt{n}_j} \mathbf{x}_j, \quad j = 1, \ldots k - 1$$ -->

<!-- Denote that every column but the first one is orthogonal and that -->

<!-- $$\mathbf{v} = (0, 0, \ldots, \underset{n_k}{\underline{1, \ldots, 1}})^T = \mathbf{1} - <\mathbf{1}, \mathbf{u}_1> \mathbf{u}_1 - \cdots - <\mathbf{1}, \mathbf{u}_{k - 1}> \mathbf{u}_{k - 1}$$ -->

<!-- It follows that -->

<!-- $$\mathbf{v} = \mathbf{1} - (1, 1, \ldots, \underset{n_k}{\underline{0, \ldots, 0}})^T = (0, 0, \ldots, \underset{n_k}{\underline{1, \ldots, 1}})^T$$ -->

<!-- Set -->

<!-- $$\mathbf{u}_k = \frac{1}{\sqrt{n_k}} \mathbf{v}$$ -->

<!-- $$ -->
<!-- X^{\ast} = \begin{bmatrix} -->
<!--   \mathbf{u}_k & \mathbf{u}_1 & \cdots & \mathbf{u}_{k - 1} -->
<!-- \end{bmatrix} \begin{bmatrix} -->
<!--   \sqrt{n_k} & <1, \mathbf{u}_1> = \sqrt{n_1} & <1, \mathbf{u}_2> = \sqrt{n_2} & \cdots & <1, \mathbf{u}_{k - 1}> = \sqrt{n_{k - 1}} \\ -->
<!--   0 & \sqrt{n_1} & 0 & \cdots & 0 \\ -->
<!--   0 & 0 & \sqrt{n_2} & \cdots & 0 \\ -->
<!--   \vdots & \vdots & \vdots & \vdots & \vdots \\ -->
<!--   0 & 0 & 0 & \cdots & \sqrt{n_{k - 1}} -->
<!-- \end{bmatrix} \equiv QR -->
<!-- $$ -->
...
<!--       \sqrt{n_k} & \sqrt{n_1} & \cdots & \sqrt{n_{k - 1}} & 1 & 0 & \cdots & 0 \\ -->
<!--     0 & \sqrt{n_1} & \cdots & 0 & 0 & 1 & \cdots & 0 \\ -->
<!--     0 & 0 & \sqrt{n_2} & \cdots & 0 & 0 & \cdots & 0 \\ -->
<!--     \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\ -->
<!--     0 & 0 & \cdots & \sqrt{n_{k - 1}} & 0 & 0 & \cdots & 1 -->
<!--     \end{array}\right] \\ -->
<!--     & \leftrightarrow \left[\begin{array}{cccc|cccc} -->
<!--       \sqrt{n_k} & 0 & \cdots & 0 & 1 & -1 & \cdots & -1 \\ -->
<!--     0 & \sqrt{n_1} & \cdots & 0 & 0 & 1 & \cdots & 0 \\ -->
<!--     0 & 0 & \sqrt{n_2} & \cdots & 0 & 0 & \cdots & 0 \\ -->
<!--     \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\ -->
<!--     0 & 0 & \cdots & \sqrt{n_{k - 1}} & 0 & 0 & \cdots & 1 -->
<!--     \end{array}\right] \\ -->
<!--     & \leftrightarrow \left[\begin{array}{cccc|cccc} -->
<!--       1 & 0 & \cdots & 0 & \frac{1}{\sqrt{n_k}} & -\frac{1}{\sqrt{n_k}} & \cdots & -\frac{1}{\sqrt{n_k}} \\ -->
<!--     0 & 1 & \cdots & 0 & 0 & \frac{1}{\sqrt{n_1}} & \cdots & 0 \\ -->
<!--     0 & 0 & 1 & \cdots & 0 & 0 & \cdots & 0 \\ -->
<!--     \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\ -->
<!--     0 & 0 & \cdots & 1 & 0 & 0 & \cdots & \frac{1}{\sqrt{n_{k - 1}}} -->
<!--     \end{array}\right] = R^{-1} -->
<!--   \end{split} -->
<!-- \end{equation*} -->
<!-- $$ -->
...
<!-- \begin{equation*} -->
<!--   \begin{split} -->
<!--     \hat{\boldsymbol\theta}^{\ast} & = R^{-1}Q^T \mathbf{Y} \\ -->
<!--     & = R^{-1} \Big( \sqrt{n_k} \sum Y_{ik}, \ldots, \sqrt{n_{k - 1}} \sum Y_{i, k - 1} \Big)^T \\ -->
<!--     & = -->
<!--   \end{split} -->
<!-- \end{equation*} -->
<!-- $$ -->
...
<!-- \begin{equation*} -->
<!--   \begin{split} -->
<!--     \hat\mu & = (\mathbf{v}^T \mathbf{v})^{-1} \mathbf{v}^T \mathbf{Y} \\ -->
<!--     & = \frac{1}{n_k}\mathbf{v}^T\mathbf{Y} \\ -->
<!--     & = \overline{Y}_{k.} = \text{baseline group mean} -->
<!--   \end{split} -->
<!-- \end{equation*} -->
<!-- $$ -->
...
$$\frac{SSR}{\sigma^2} \hsim \chi^2(k - 1) \ind \frac{SSE}{\sigma^2} \sim \chi^2(N - (k - 1) - 1)$$

and so

$$F_0 = \frac{SSR / (k - 1)}{SSE / (N - k)} \hsim F(k - 1, N - k)$$
...
\begin{equation}
  Y_i \indep N(\beta_0 + \beta_1 x_{i1} + \cdots \beta_p x_{ip}, \sigma^2)
  (\#eq:ynormal)
\end{equation}
...
\begin{equation}
  \begin{split}
    ...
    & = (2 \pi \sigma^2)^{-\frac{n}{2}} \exp\Big( -\frac{1}{2\sigma^2} \lVert \mathbf{Y} - X \B \rVert^2 \Big) \\
    & \equiv L(\B, \sigma^2)
  \end{split}
  (\#eq:multlike)
\end{equation}
...
\begin{equation}
  \begin{split}
    l(\B, \sigma^2) & = \ln L(\B, \sigma^2) \\
    & = - \frac{n}{2} \ln (2 \pi \sigma^2) - \frac{1}{2 \sigma^2} \lVert \mathbf{Y} - X \B \rVert^2
  \end{split}
  (\#eq:multloglk)
\end{equation}
...
```{remark, name = "Likelihood Equation"}
Since $l = - \frac{n}{2} \ln (2 \pi \sigma^2) - \frac{1}{2 \sigma^2} (\mathbf{Y} - X \B)^T (\mathbf{Y} - X \B)$,

\begin{enumerate}
  \item $\frac{\partial l}{\partial \B} = \frac{1}{\sigma^2} X^T (Y - X \B) = 0$
  \item $\frac{\partial l}{\partial \sigma^2} = - \frac{n}{2 \sigma^2} + \frac{1}{2 \sigma^4} \lVert \mathbf{Y} - X \B \rVert^2 = 0$
\end{enumerate}
```
...
\begin{equation}
  \begin{cases}
    \hb^{MLE} = (X^T X)^{-1} X^T \mathbf{Y} = \hb^{LSE} \\
    \hat\sigma^{2 MLE} = \frac{\lVert \mathbf{Y} - X \hb \rVert^2}{n} \neq \hat\sigma^{2 LSE}
  \end{cases}
  (\#eq:mulmlvsls)
\end{equation}
...
Since $\hb^{MLE} = \hb^{LSE}$, $\hb^{MLE}$ is also an *ubiased estimator*, i.e.

$$E\hb = \B$$

Extending Theorem \@ref(thm:rclb2), we can see if this unbiased estimator has the *minimum variance*.

```{theorem, rclbm, name = "Rao-Cramer Lower Bound, multivariate case"}
Let $X_1, \ldots, X_n \stackrel{iid}{\sim} f(x ; \theta1, \ldots, \theta_{p + 1})$ and let $\boldsymbol{\theta} = (\theta_1, \ldots, \theta_{p + 1})^T$. If $\boldsymbol{\hat\theta}$ is an unbiased estimator of $\boldsymbol\theta$, then
...
\begin{equation}
  \frac{\partial l}{\partial \B} = \frac{1}{\sigma^2} X^T (Y - X \B) \quad \text{and} \quad \frac{\partial^2 l}{\partial \B \partial \B^T} = - \frac{1}{\sigma^2} X^T X
  (\#eq:loglikderiv)
\end{equation}
...
\begin{equation}
  I_n(\B) = - E\bigg[ \frac{\partial^2 l}{\partial \B \partial \B^T} \bigg] = \frac{1}{\sigma^2} X^T X
  (\#eq:multinform)
\end{equation}
...
\begin{equation}
  I_n^{-1}(\B) = \sigma^2 (X^T X)^{-1} = Var(\hb^{LSE})
  (\#eq:multinforminv)
\end{equation}

Hence, $\hb^{MLE} = \hb^{LSE}$ is the *minimum variance unbiased estimator*. In fact, it is same when $\sigma^2$ is unknown. Compute derivatives for $\sigma^2$ additionaly.

\begin{equation}
  \frac{\partial l}{\partial \sigma^2} = - \frac{n}{2 \sigma^2} + \frac{1}{2 \sigma^4} \lVert \mathbf{Y} - X \B \rVert^2 \quad \text{and} \quad \frac{\partial^2 l}{\partial^2 \sigma^2} = \frac{n}{2\sigma^4} - \frac{1}{\sigma^6} \lVert \mathbf{Y} - X \B \rVert^2
  (\#eq:loglikederiv2)
\end{equation}
...
\begin{equation}
  I_n(\B, \sigma^2) = \left[\begin{array}{c|c}
    \frac{\partial^2 l}{\partial \B \partial \B^T} & \frac{\partial^2 l}{\partial \B \partial \sigma^2} \\ \hline
    \frac{\partial^2 l}{\partial \B^T \partial \sigma^2} & \frac{\partial^2 l}{\partial^2 \sigma^2}
  \end{array}\right]
  (\#eq:blockinform)
\end{equation}
...
\begin{equation}
  I_n^{-1}(\B, \sigma^2) = \left[\begin{array}{c|c}
    I_n^{-1}(\B) & \vdots \\ \hline
    \cdots & I_n^{-1}(\sigma^2)
  \end{array}\right]
  (\#eq:blockinforminv)
...
In other words, it is still valid that $I_n^{-1}(\B)$ implies minimum variance of $\hb$.
