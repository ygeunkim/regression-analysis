...
We have already seen in section \@ref(simplebdist) that the estimators $\hat\beta_0$ and $\hat\beta_1$ are random variables. So $\hat\mu_x$ is. In fact, the ribbon of the line in Figure \@ref(fig:smoothline) represents $+ \widehat{SE}(\hat\mu_x)$ and $- \widehat{SE}(\hat\mu_x)$. It can be drawn by default with the option of the `geom_smooth(se = TRUE)`.
