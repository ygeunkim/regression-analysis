...
Denote that Equation \@ref(eq:ssq) is quadratic. Then we can catch its minimum by finding the zero point of the first derivative. Set
...
```{proof}
$\ref{itm:respropone}$ Recall that this is the assumption of the regression model.
...
Now consider the *residual sum of squares* $SSE = \sum (Y_i - \hat{Y_i})^2$ in the definition \@ref(def:sse). As mentioned, this is related to the *prediction errors*, which the regression model could not explain the data.
...
$$\widehat{SE}(\hat\mu_x) = \hat\sigma \sqrt{\bigg( \frac{1}{n} + \frac{(x - \overline{x})^2}{S_{XX}} \bigg)}$$
...
$$\widehat{SE}(\hat{Y_x} - Y_x) = \hat\sigma \sqrt{\bigg( 1 + \frac{1}{n} + \frac{(x - \overline{x})^2}{S_{XX}} \bigg)}$$
...
```{proof}
$\ref{itm:idemone}$ and $\ref{itm:idemtwo}$ are trivial.
...
\begin{enumerate}[label=(\roman*)]
  \item If $A$ and $B$ are symmetric, $$Y^T AY \ind Y^T BY \Leftrightarrow A\Sigma B = 0$$
  \item If $A$ is symmetric, $$Y^T AY \ind BY \Leftrightarrow B\Sigma A = 0$$
\end{enumerate}
```

```{theorem, quaddist, name = "Distribution of quadratic form"}
Assume that $\mathbf{Y} \sim MVN(\mathbf\mu, I)$ and that $A$ is symmetric and idempotent. Then
