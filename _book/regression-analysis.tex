...
\[
Var(\boldsymbol{\hat\beta}) = \sigma^2 diag\bigg(\frac{1}{r_{11}^2}, \ldots, \frac{1}{r_{p + 1, p + 1}^2} \bigg)
\]
...
\BeginKnitrBlock{remark}[$(1 - \alpha)100\%$ Confidence interval]
\iffalse{} {Remark (\((1 - \alpha)100\%\) Confidence interval). } \fi{}By QR decomposition for \(X = QR\), \((1 - \alpha)100\%\) confidence interval for each individual \(\beta_j\) can be computed as
...
\[T_0 = \frac{\hat\beta_k - \alpha_k}{\hat\sigma\sqrt{c_{kk}}} = \frac{\hat\beta_k - \alpha_k}{\hat\sigma / r_{k + 1, k + 1}} \sim t(n - p - 1)\]

where \((X^TX)^{-1} = (c_{ij})_{0 \le i,j \le p}\) and \((R^TR)^{-1} = diag\Big(\frac{1}{r_{ii}^2}\Big)_{1 \le i \le p +1}\)
\EndKnitrBlock{theorem}
