...
\BeginKnitrBlock{proposition}[Distribution of SSE]
\protect\hypertarget{prp:mssedist}{}{\label{prp:mssedist} \iffalse (Distribution of SSE) \fi{} }\[\frac{SSE}{\sigma^2} \sim \chi^2(n - p - 1)\]
\EndKnitrBlock{proposition}
...
\(\delta\) proof is exactly same as Proposition \ref{prp:ssedist}. Hence, \(\delta = 0\).
\EndKnitrBlock{proof}

\BeginKnitrBlock{proposition}[Distribution of SSR]
\protect\hypertarget{prp:mssrdist}{}{\label{prp:mssrdist} \iffalse (Distribution of SSR) \fi{} }\[\frac{SSR}{\sigma^2} \sim \chi^2(p, \delta)\]

where \(\delta = \frac{\boldsymbol{\hat\beta}^TX^T(\Pi_X - \Pi_{\mathbf{1}})X\boldsymbol{\hat\beta}}{\sigma^2}\)
\EndKnitrBlock{proposition}
