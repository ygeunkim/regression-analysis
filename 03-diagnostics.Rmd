# Model Adequacy and Regression Diagnostics {#regdiag}
...
In this case of Figure \@ref(fig:qqchisq), $r_i$ is observed as heavier right tail than left. This is called *positive skew* or *right skew*, i.e. its mass leans to the left.
...
See Figure \@ref(fig:qqskewn). Left tail is observed heavier than right. This is called *negative skew* or *left skew*. Its mass leans to the right.
...
```{theorem, hbnoithm}
Regression coefficient without $i$-th observation can come from the original design matrix. Also, it is affected by $i$-th residual.
...
Cook's distance is related to the distance between $\hb$ and $\hb_{(-i)}$.
